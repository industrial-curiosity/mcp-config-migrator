...
Between the write-confirmation prompt and the write itself, `flow.ts` calls `src/cli/backupFlow.ts`'s `maybeBackup`, which reads the persisted `BackupPreference` (`src/model/versionsStore.ts`) and either appends a version silently (`alwaysOn`/`alwaysOff`) or prompts with four options — "Yes" / "Yes, always" / "No" / "No, never" — where only the two "always" choices persist a new preference. The write itself then calls `targetAdapter.save()` directly; there is no whole-file backup anymore.
