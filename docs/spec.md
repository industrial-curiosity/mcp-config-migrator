...
`mcp-config-migrator` is a one-shot interactive CLI (`npx mcp-config-migrator`) that migrates MCP server entries between VS Code, Cursor, Claude Code, Codex, and Pi config files. There is no daemon, watch mode, or two-way sync — each run reads a source and a target config, lets the user resolve any conflicts, optionally manage merged servers one at a time before the summary, and writes the merged result back to the target.
...
- `summary.ts` — counts and server names per category (added, unchanged, skipped, conflicts by resolution including merges) for the pre-write summary, and `isNoOp` to detect when a migration would change nothing. `ManualEdits` (`{ created: Set<string>; edited: Set<string>; skipped: Set<string> }`) is an optional third argument to `summarize()`; when present, it reclassifies entries that were manually created, edited, or skipped in the pre-summary edit step — created entries appear in `added`, skipped entries appear in `skipped`, edited unchanged/conflict-accept entries move to `conflicts.merged`, and edited add entries remain in `added`.

### CLI (`src/cli/`)

`src/cli/index.ts` is the published `bin` entry (`dist/cli.js`, shebang `#!/usr/bin/env node`). It parses `process.argv.slice(2)` via `src/cli/args.ts`'s pure `parseArgs` and dispatches to migration (no args), direct editing (`edit`), `restore [--file|-f <path>]`, `config backup`, or help (`--help`/`-h`/`/?`).

`src/cli/flow.ts` is the interactive migrate flow, built on `@clack/prompts`; `src/cli/editFlow.ts` is the direct-edit flow. They share IDE/scope/path selection and the pre-write server manager. Any cancellation (Ctrl+C / clack's cancel) anywhere before the write-confirmation prompt aborts with no file writes. `src/cli/cancel.ts` holds the shared `CliCancelled` error, `unwrap`, and `withCancelHandling` helpers used by these flows and the backup/restore flows alike.
...
After all conflicts are resolved and `applyMerge` produces the merged config, `src/cli/editStep.ts`'s `editMergedServers` presents a single-server menu over the merged config, returning to the updated list after every add, edit, or deletion. The menu keeps **Add a server** immediately above **Finish editing**, even for an empty config. Adding prompts for a unique name and transport, then opens a transport-specific normalized JSON template; command and URL completeness are not required at this stage. Editing opens the editor with a skip-instruction header followed by the selected server's normalized JSON (without `name` or `extra`). The same fix-or-redo validation loop is used. If the user clears the file (empty, whitespace-only, or just `{}`), the server is removed from the merged config and recorded as skipped — `isSkipSignal` implements this check and runs before `parseMergedServer` so that `{}` is never presented as a validation error. The function returns `{ updatedConfig, manualEdits }` where `manualEdits` carries the `created`, `edited`, and `skipped` sets; `flow.ts` passes `manualEdits` to `summarize()` and uses `updatedConfig` (not the pre-edit `merged`) for all subsequent writes. A migration with no imported changes still enters this manager so the user can add a server; it exits without writing when no manual changes are made.
