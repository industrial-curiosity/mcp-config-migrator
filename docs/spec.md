...
After all conflicts are resolved and `applyMerge` produces the merged config, `src/cli/editStep.ts`'s `editMergedServers` presents an optional single-server menu over the merged config, returning to the updated list after every edit or deletion. It opens the editor with a skip-instruction header followed by the selected server's normalized JSON (without `name` or `extra`). The same fix-or-redo validation loop is used. If the user clears the file (empty, whitespace-only, or just `{}`), the server is removed from the merged config and recorded as skipped — `isSkipSignal` implements this check and runs before `parseMergedServer` so that `{}` is never presented as a validation error. The function returns `{ updatedConfig, manualEdits }` where `manualEdits` carries the `edited` and `skipped` sets; `flow.ts` passes `manualEdits` to `summarize()` and uses `updatedConfig` (not the pre-edit `merged`) for all subsequent writes.
